...
#include "include/geAnimation.hpp"

void geAnimation::updateDeltas() {
    if (deltaTimeSingleMove > 0) {
        dX = std::abs(currentDestionationPoint.x - currentSourcePoint.x) / deltaTimeSingleMove;
        dY = std::abs(currentDestionationPoint.y - currentSourcePoint.y) / deltaTimeSingleMove;
        dZ = std::abs(currentDestionationPoint.z - currentSourcePoint.z) / deltaTimeSingleMove;
    }
}
...
geAnimation::geAnimation(double iSpan, unsigned int iType) {
    if (iType != 1) {
...
void geAnimation::insertPoint(GLdouble ix, GLdouble iy, GLdouble iz) {
    ...
    if (numberOfControlPoints != 0) {
        ...
        deltaTimeMillis = ((span / numberOfControlPoints) * 1000);

        /* TODO: fix the 16 value for the animation check glfw */
        deltaTimeSingleMove = ((span / numberOfControlPoints) * 1000 / 16);
...
double geAnimation::getDeltaTime() {
    return (this->deltaTimeMillis);
}

void geAnimation::updateAnimation(double timePassed) {
